...
public class GUIMapActivitySelectController : MonoBehaviour
{
	...
	private bool nextRegionUnlocked()
	{
		...
		int completedTutorialIndex = player.getCompletedTutorialIndex();
		int areaRegion = player.getAreaRegion();
		if (gameData.checkFeatureIsUnlocked(gameLockSet, "REGION2UNLOCK", completedTutorialIndex) || gameData.checkFeatureIsUnlocked(gameLockSet, "REGION3UNLOCK", completedTutorialIndex) || gameData.checkFeatureIsUnlocked(gameLockSet, "REGION4UNLOCK", completedTutorialIndex) || gameData.checkFeatureIsUnlocked(gameLockSet, "REGION5UNLOCK", completedTutorialIndex))
		{
			return true;
		}
		return false;
