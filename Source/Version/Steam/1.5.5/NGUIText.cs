...
public static class NGUIText
{
	...
	public static bool ParseSymbol(string text, ref int index, BetterList<Color> colors, bool premultiply, ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike)
	{
		...
		if (text[index + 7] == ']')
		{
			...
			if (EncodeColor24(color) != text.Substring(index + 1, 6).ToUpper(CultureInfo.InvariantCulture))
			{
				return false;
			}
			if (colors != null)
		...
		if (text[index + 9] == ']')
		{
			...
			if (EncodeColor32(color2) != text.Substring(index + 1, 8).ToUpper(CultureInfo.InvariantCulture))
			{
				return false;
			}
			if (colors != null)
